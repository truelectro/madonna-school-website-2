...
export function HeroSection({ data }: { data: any }) {
    const heroImageUrls: string[] = data.backgroundImages && Array.isArray(data.backgroundImages)
        ? data.backgroundImages.map((img: any) => urlFor(img).url())
        : [];
                        ...
                        <>
                            {heroImageUrls.map((url, index) => (
                                <Image
                                    key={index}
                                    src={url}
                                    alt={data.heading || `Hero Image ${index + 1}`}
                                    fill
